...
def voice_response(update, context):
    ...
    tts = gTTS(text=text, lang='en', tld='co.id')  # Create a gTTS object with the text
    tts.save('response.wav')  # Save the generated voice as an wav file
    context.bot.send_voice(chat_id=update.effective_chat.id, voice=open('response.wav', 'rb'))
